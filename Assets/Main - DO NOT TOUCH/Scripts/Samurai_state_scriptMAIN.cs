...
public class Samurai_state_scriptMAIN : MonoBehaviour
{
    ...
    {
        current_hp = max_hp;
        MainContextMAIN.player = this.gameObject;
        //TakeDamage(1);
    }
    ...
    {
        AudioManagerMAIN.instance.PlaySFX("GameOver");
    ...
    public void Heal()
    {
        ...
        hp_bar.GetComponent<Health_bar_script>().HealHeart();
        current_hp++;
    }
    public void TakeDamage(int damage)
    {
        //Debug.Log("damage_taken");
        if (invisibility || dead || Upgrade_managerMAIN.IsUpgradePicking()) return;

        hp_bar.GetComponent<Health_bar_script>().MinusHeart();

        if (Frostring != null) Instantiate(Frostring, transform.position, Quaternion.identity);

        current_hp -= damage;
        if (current_hp <= 0) Death();
        else
        {
            AudioManagerMAIN.instance.PlaySFX("PlayerDamaged");
